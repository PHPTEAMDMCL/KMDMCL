...
.row-fluid.sortable
  .box.span12
    ...
    .box-content
      form.form-horizontal(method='POST', action='/admin/landing/editlivepost/'+$live['id'])
        fieldset
          .control-group
            label.control-label(for='focusedInput') Tên chương trình
            .controls
              input#focusedInput.input-xlarge.focused(type='text', name='name', placeholder='Tên chương trình', value=$live['name'])
          ...
          .control-group
            ...
            .controls
              @if($live['status']==0)
              label.radio
                input#optionsRadios1(type='radio', name='status', value='0', checked='')
                | Không kích hoạt
              label.radio
                input#optionsRadios2(type='radio', name='status', value='1')
                | Kích hoạt
              @else
              label.radio
